...
#------------------------
# numbers of students that apply to the same school = 



#------------------------
# Apply to less than 6 choices 0 

sum(is.na(datstu[11:16]))
which(is.na(datstu[11:16]))
...
eg2_data <- left_join(datstu_for_merging, school_name_for_merging, 
...
eg2_data <- left_join(eg2_data, cutoff_quality_merging, 
                      by =  c('schoolcode' =  'schoolcode', 
                              'choicepgm' = 'choicepgm',
                              'rank' = 'rank')  )

nstudents_cutoff_quality_in_one_program <- eg2_data %>%
  group_by(schoolcode, choicepgm) %>%
  dplyr::summarise(nstudents_programlevel = sum(nstudents),
                   cutoff = min(cutoff),
                   quality = sum(quality_sum)/ sum(nstudents))

school_program_level_data <- left_join(nstudents_cutoff_quality_in_one_program, school_name_for_merging,
                                       by =  c('schoolcode' =  'schoolcode') )
...
#datstu_school_program_jssdistrict <-  select(datstu_school_program_jssdistrict,
#                                             -c(rank, nstudents, X))

datstu_school_program_jssdistrict <- distinct(datstu_school_program_jssdistrict)

eg3_school_program_level_data<- left_join( school_program_level_data,
...
nrow(eg3_school_program_level_data)
...
# report the average and sd for each ranked choice 

eg4_data_cutoff <- eg2_data %>%
  group_by(rank) %>%
  dplyr::summarise(
    avg_cutoff = mean(cutoff),
    sd_cutoff = sd(cutoff)
  )

eg4_data_quality <- eg2_data %>%
  group_by(rank) %>%
  dplyr::summarise(
...
eg4_data_distance <- eg3_school_program_level_data %>%
  group_by(rank)%>%
  dplyr::summarise(
...
library('margins')
te = glm(yvar~x1+x2+x3, family = binomial(link = "probit"),data = dat_samp)
